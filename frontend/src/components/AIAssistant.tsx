...
export default function AIAssistant() {
  ...
  return (
    <div className={`fixed top-0 right-0 bottom-0 z-40 w-80 bg-white dark:bg-[#0F1626] border-l border-[#E2E8F0] dark:border-[#1E293B] shadow-2xl transition-transform duration-300 transform flex flex-col justify-between ${
      isOpen ? 'translate-x-0' : 'translate-x-full'
    }`}>
      {/* Toggle Tab */}
      <button
        onClick={() => setIsOpen(!isOpen)}
        className="absolute left-[-40px] top-1/2 -translate-y-1/2 bg-blue-600 hover:bg-blue-700 text-white rounded-l-xl py-3 px-2 shadow-lg cursor-pointer flex flex-col items-center gap-1.5"
      >
        <span className="text-[10px] font-bold uppercase tracking-widest writing-mode-vertical rotate-180">AI Helper</span>
      ...
      <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-4">
        ...
        {/* Chat History Panel */}
        <div className="flex-1 flex flex-col gap-3 max-h-[300px] overflow-y-auto border-b border-[#E2E8F0] dark:border-[#1E293B] pb-4">
          {messages.map((msg, i) => (
            <div key={i} className={`flex flex-col max-w-[85%] ${msg.sender === 'user' ? 'self-end items-end' : 'self-start'}`}>
              ...
              {msg.actionPlan && msg.actionPlan.length > 0 && (
                <div className="mt-2 bg-[#FBBF24]/10 border border-[#FBBF24]/30 rounded-lg p-2.5 flex flex-col gap-1 w-full text-[10px] font-semibold text-[#D97706]">
                  <span className="uppercase text-[8px] font-bold tracking-wider mb-1">Generated Action Plan Checklist:</span>
                  {msg.actionPlan.map((step, idx) => <span key={idx}>{step}</span>)}
                </div>
              )}
            </div>
          ))}
          {isTyping && <span className="text-[10px] text-[#94A3B8] italic">AI is typing...</span>}
          <div ref={chatEndRef} />
        </div>

        {/* Legal Language Translator block */}
        <div className="flex flex-col gap-2">
          <span className="text-[10px] font-bold uppercase text-[#94A3B8] tracking-wider">Legal Code Translator</span>
          <textarea
            className="form-control !text-[11px] h-16 resize-none p-2"
            placeholder="Paste confusing rules here..."
            value={legalInput}
          ...
          <button 
            ...
            className="btn btn-secondary !py-1 text-[10px] w-full"
          >
            Translate to Simple Terms
          </button>
          {translatedText && (
            <div className="p-3 bg-emerald-500/5 border border-emerald-500/20 text-emerald-600 dark:text-emerald-400 rounded-lg text-[10px] font-medium leading-relaxed">
              {translatedText}
            </div>
          )}
        </div>
      ...
      >
        ...
        />
        <button type="submit" className="btn btn-primary h-8 w-8 !p-0 rounded-xl shrink-0">
